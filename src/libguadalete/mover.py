def LoadFunctions(clips):
    mod_mover = clips.BuildModule("MOVER",
                                  "(import MAIN deftemplate initial-fact ficha-r dimension tiempo mueve turno) (import MAIN deffunction ?ALL)")

    movimiento = mod_mover.BuildRule("movimiento", "(declare (salience 90)) (tiempo ?t) ?h1 <- (mueve (num ?n) (mov ?m) (tiempo ?t)) ?h2 <- (ficha-r (equipo ?e) (num ?n) (puntos ?p) (pos-x ?x) (pos-y ?y) (descubierta ?d)) (dimension ?dim) (test (mov-valido ?dim ?m ?x ?y)) (not (ficha-r (pos-x ?x2&:(= (+ ?x (mov-x ?m)) ?x2)) (pos-y ?y2&:(= (+ ?y (mov-y ?m)) ?y2)))) (not (movido ?e ?t)) (turno ?e)",
                                         '(retract ?h1 ?h2) (printout t "Movimiento de " ?e", "?n"(puntos "?p") :mov "?m crlf) (assert (movido ?e ?t)) (assert (ficha-r (equipo ?e) (num ?n) (puntos ?p) (pos-x (+ ?x (mov-x ?m))) (pos-y (+ ?y (mov-y ?m))) (descubierta ?d)))')
